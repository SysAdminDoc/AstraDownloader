...
browser cookies into Astra Downloader 2.15.1 for authenticated yt-dlp downloads.
...
Astra Downloader 2.15.1 uses yt-dlp's site extractors, not only
...
Astra Downloader 2.15.1 also has a second, separate cookie path: **site
...
The locally built package also pins `yt-dlp==2026.8.19` in
...
| A public hostname resolves to a private address (DNS rebinding against the URL policy). | Not fully mitigated. The policy is literal-only by design because resolving at validation time proves nothing about resolution a millisecond later. A name pointed at RFC1918 space still reaches yt-dlp. The loopback listener and bearer token limit who can submit requests; they do not constrain their network destination. Cookies remain scoped to the selected site, not to YouTube alone. A hostname that resolves privately can still receive cookies valid for that hostname. |
...
Site usernames, passwords and one-off video passwords can be passed to yt-dlp on its command line. Processes running as the same Windows user may be able to read them. Redaction in the API, history and diagnostics does not remove this process-level exposure. Cookies may be sent to the site being downloaded from; they do not remain only on disk.
