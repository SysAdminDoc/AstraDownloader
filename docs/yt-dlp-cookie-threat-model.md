...
YouTube cookies into Astra Downloader 2.11.0 for authenticated yt-dlp downloads.
...
Astra Downloader 2.11.0 downloads from any public site yt-dlp supports, not only
...
Astra Downloader 2.11.0 also has a second, separate cookie path: **site
...
5. `DownloadManager._run_download()` passes the jar with `--cookies <path>`,
   ...
   Downloader 2.11.0 downloads from any public site, so this scoping keeps a
