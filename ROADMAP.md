...
- [ ] P0 — AD-02 — Compute the winget `InstallerSha256` from the staged artifact and make the gate compare it
  Why: the 2.7.0 manifest declares `0982ff61c3776a3d24aa98a38c3b657ed01c55dcfd2f34ebba6f0d023c8bda20`, a digest that appears nowhere else in the repository and matches no built artifact (the exe in the tree is `e321d362…`). `scripts/check-versions.js:77` only asserts the value is 64 hex digits, so the gate certifies a field it never checks.
  Evidence: `packaging/winget/manifests/s/SysAdminDoc/AstraDownloader/2.7.0/…installer.yaml:11`; `scripts/check-versions.js:60-84`; `AstraDownloader.exe.sha256`.
  Touches: `scripts/check-versions.js`, `scripts/stage-companion-release.js`, `packaging/winget/manifests/`
  Acceptance: the manifest digest is generated by the staging script; `check:versions` fails when the manifest digest differs from the staged artifact's, proven by a test that mutates one byte of the digest.
  Complexity: S

- [ ] P0 — AD-03 — Force the test suite offscreen by default
  Why: `QT_QPA_PLATFORM=offscreen` is set only inside `env=` dicts handed to `subprocess.run`. Neither `pytest.ini` nor `conftest.py` sets it, and there is no CI, so a plain `py -3.12 -m pytest` builds a real `QApplication` and runs eight `.show()` calls on the operator's physical display.
  Evidence: `pytest.ini`; `astra_downloader/conftest.py`; `test_astra_downloader.py:11044-11053` and its eight `.show()` sites; `CLAUDE.md:34` claims offscreen "in CI" and there is no CI.
  Touches: `astra_downloader/conftest.py`, `pytest.ini`, `CLAUDE.md`
  Acceptance: `py -3.12 -m pytest` opens no window; an explicit opt-out env var still allows an on-screen run; the CLAUDE.md line is corrected.
  Complexity: S

- [ ] P0 — AD-04 — Give Deno a security floor separate from its yt-dlp functional floor
  Why: `DENO_MIN_VERSION = "2.3.0"` is yt-dlp's EJS requirement and is also what triggers a refresh, so an install that provisioned Deno 2.3.0–2.8.0 keeps it forever. Eight Deno advisories published 2026-05-27 are fixed above it — CVE-2026-44726 in 2.7.8, CVE-2026-49401 in 2.7.14, CVE-2026-49440 in 2.8.1.
  Evidence: `astra_downloader/health.py:48`; refresh trigger at `astra_downloader/astra_downloader.py:1655`; GHSA-chqv-56wv-7564 / GHSA-8xpq-cjcf-3wh9 / GHSA-9xg4-qhm4-g43w. The repo already models this split for ffmpeg (`_FFMPEG_MIN_VERSION` + `_FFMPEG_MIN_SNAPSHOT_DATE`).
  Touches: `astra_downloader/health.py`, `astra_downloader/astra_downloader.py`
  Acceptance: a planted Deno reporting 2.7.0 is refreshed; the readiness row names "below the security floor" distinctly from "below the runtime floor"; a test pins both floors.
  Complexity: S
...
- [ ] P1 — AD-14 — Take the parse out of the JS-runtime probe's `try`
  Why: `evaluate_javascript_runtime` wraps both `runner(...)` and `parse_javascript_runtime_version` in one handler, so a parser bug is reported as `runtime-probe-failed`. Only the subprocess call can legitimately fail. This is the identical shape to the 2026-08-06 QuickJS re-download bug that CLAUDE.md records with an explicit "Don't".
  Evidence: `astra_downloader/health.py:635-641`; `CLAUDE.md:104-114`. Related: `_probe_ytdlp_binary` (`astra_downloader.py:2233`) uses `except Exception: return ''` while its sibling `_probe_companion_version` (`:3500`) correctly narrows.
  Touches: `astra_downloader/health.py`, `astra_downloader/astra_downloader.py`
  Acceptance: only the subprocess call sits inside the handler; a fault injected into the parser surfaces as a distinct reason, not `runtime-probe-failed`.
  Complexity: S
...
- [ ] P2 — AD-26 — Add a staging step between paste and queue
  Why: pasting a channel or playlist commits blind. JDownloader's LinkGrabber resolves links into a staging area with title, duration, size and availability per entry, deduplicates, and lets the user prune before anything is queued. No yt-dlp desktop GUI has this, and the pieces already exist here — `preview_playlist`, the format probe, and the subscription archive keys.
  Evidence: RESEARCH.md "Competitive Landscape" (JDownloader, YTDLnis per-item editing); `astra_downloader/download.py:6579` `preview_playlist`; `routes.py:762` `/playlist`.
  Touches: `astra_downloader/gui_download_page.py`, `gui.py`, `download.py`, `routes.py`
  Acceptance: a pasted playlist resolves into a reviewable list; per-item and batch-apply edits both work; items already in the archive are flagged; only committed items enter the queue.
  Complexity: L

- [ ] P2 — AD-27 — Report the pipeline as steps, not one blended percentage
  Why: a job is fetch → merge → embed → (transcribe), and a single percentage hides which step is slow or stuck — the "silently hangs" complaint class across every competitor's tracker. cobalt 11.0's rewrite made per-step progress its headline UX claim.
  Evidence: RESEARCH.md "Market signal" T5; cobalt discussion 1288; `astra_downloader/download.py:5144` `_run_download`.
  Touches: `astra_downloader/download.py`, `gui.py`, `routes.py`
  Acceptance: the queue row shows the current named step and its own progress; `/status/<id>` and `/queue` expose the step; a stalled step is distinguishable from a slow one.
  Complexity: M

- [ ] P2 — AD-28 — Show the yt-dlp command a job actually ran
  Why: the product's value is diagnosis, and the one thing it never shows is the argv it built from 77 settings, a site profile and a cookie jar. ytdl-sub shipped `inspect --match` for exactly this, and it is the missing affordance in every GUI in the segment.
  Evidence: RESEARCH.md "Competitive Landscape" (ytdl-sub); `astra_downloader/download.py` argv builders; `validate_ytdlp_spawn_args`.
  Touches: `astra_downloader/gui.py`, `gui_download_page.py`, `download.py`, `astra_downloader.py` (`redact_diagnostic_text`)
  Acceptance: a failed (and a completed) job exposes its redacted argv in the UI and in the diagnostics bundle; credentials, tokens and cookie paths are redacted by the existing helper; a test asserts no secret survives.
  Complexity: M
