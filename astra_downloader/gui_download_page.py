...
class DownloadPageMixin:
    def _build_download(self):
        ...
        self.quick_download_url.textChanged.connect(self._sync_playlist_staging_button)
        url_row.addWidget(self.quick_download_url, 1)
        self.btn_quick_stage = self._make_tool_button("Review playlist", "ghost")
        self.btn_quick_stage.setToolTip(
            tr("Preview and select videos in this playlist before downloading.")
        )
        self.btn_quick_stage.clicked.connect(self._open_playlist_staging)
