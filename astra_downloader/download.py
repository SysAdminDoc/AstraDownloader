...
DOWNLOAD_FAILURE_RECOVERY = {
    'drm-protected': {
        'error': (
            'This video is encrypted with DRM, so no downloader can produce a '
            'file from it.'
        ),
        'advice': (
            'This is not a fault to retry or a sign-in to fix. The Sites page '
            'marks which services serve DRM-protected media.'
        ),
        'next_action': 'nothing-to-retry',
    },
    'media-unavailable': {
        'error': (
            'The source no longer has this video. It may have been removed, '
            'made private, or the live event may have ended.'
        ),
        'advice': (
            'Check the link in a browser. If the page is gone, there is '
    ...
    'blocked-by-site': {
        ...
        'advice': (
            'Set a browser to imitate in Settings. This is usually the fix '
    ...
    'rate-limited': {
        ...
        'advice': (
            ...
            "request pacing in Settings. A bandwidth cap won't help here. "
...
def write_cookies_netscape(cookies, target_path, *, logger=None, domain_filter=None):
    ...
    lines = [
        ...
        "# Generated by Astra Downloader. Do not edit this file.",
        "",
    ]
...
def _classify_failure_text(text, aggregated=False):
    ...
    # Before the network bucket: a video that no longer exists is not a broken
    ...
    if not aggregated and any(marker in text for marker in (
        ...
        'this video does not exist', 'content is no longer',
    )):
...
class DownloadManagerCore:
    ...
    def _evaluate_recovery_precondition(self, dl, requirement):
        ...
        if requirement == 'impersonate':
            ...
            if not target:
                return False, (
                    'Choose a browser to imitate in Settings. This usually fixes '
                    'the refusal. Then retry.'
                )
            with self._readiness_lock:
